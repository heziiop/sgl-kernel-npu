...
static const std::map<SFALayout, size_t> SFA_LAYOUT_DIM_MAP = {
    {SFALayout::BSND, DIM_NUM_FOUR}, {SFALayout::TND, DIM_NUM_THREE}, {SFALayout::PA_BSND, DIM_NUM_FOUR},
...
void SFAMlaTiling::GenTilingKey()
{
    ...
    tilingKey_ = GET_TPL_TILING_KEY(0U, pageAttention, layoutQuery, layoutKV, perfMode_ == SFAPerfMode::V_TEMPLATE_MODE,
                                    static_cast<uint32_t>(sfaInfo_->gSize > 64));  // N1 > 128时核间切G
...
void SFAMlaTiling::CalcUbBmm()
{
    ...
    // Kernel writes 16-element aligned tiles, so tiling reserves the same size.
...
void SFAMlaTiling::CalcInnerSize(uint32_t s2Size)
{
    ...
    // For flash decode, split the 256-to-512 range into two chunks so the
    // double-buffered path has two iterations.
...
void SFAMlaTiling::FillTilingSplitKVMla()
{
    ...
    tilingData_.splitKVParams.set_accumOutSize(aicNum_ * 2 * sfaInfo_->n2Size * mBaseSize_ *
                                               headDimAlign_);  // head and tail reduction per core
    tilingData_.splitKVParams.set_logSumExpSize(2 * aicNum_ * 2 * sfaInfo_->n2Size *
                                                mBaseSize_ *  // head/tail reductions; sum + max
...
{
    return actCoreNum * 2 * sfaInfo_->n2Size * mBaseSize_;  // head and tail reductions per core
}
...
void SFAMlaTiling::NormalCalcFDWorkSpace(const uint32_t actCoreNum)
{
    if (splitKVFlag_) {
        ...
        uint32_t FDParamNums = CalcBalanceFDParamNums(
            actCoreNum);  // balanceModeFlag_ ? CalcBalanceFDParamNums(actCoreNum) : CalcUnbalanceFDParamNums();
        accumOutSize = FDParamNums * headDimAlign_;
        logSumExpSize = 2 * FDParamNums * (BYTE_BLOCK / sfaInfo_->blockTypeSize);  // log and sum use equal storage
...
void SFAMlaTiling::GetWorkspaceSize()
{
    ...
    if (sfaInfo_->isA5) {
        ...
        workspaceSize_ +=
            4 * 128 * 4 * (2 * actCoreNum);  // cache MTE2 size: 128 entries, 512-byte alignment, two AIV cores
    }
...
ge::graphStatus SFATilingCheck::GetExpectedShape(gert::Shape &shapeExpected, const SFATilingShapeCompareParam &param,
...
ge::graphStatus SFATilingCheck::CompareShape(SFATilingShapeCompareParam &param, const gert::Shape &shape,
                                             const SFALayout &layout, const std::string &name) const
{
    ...
    if (shape.GetDimNum() != shapeExpected.GetDimNum()) {
        OP_LOGE(opName_, "%s dimension is %zu, expected dimension is %zu.", name.c_str(), shape.GetDimNum(),
                shapeExpected.GetDimNum());
        return ge::GRAPH_FAILED;
    }
    ...
    for (size_t i = 0; i < shape.GetDimNum(); i++) {
        if (shape.GetDim(i) != shapeExpected.GetDim(i)) {
            OP_LOGE(opName_, "%s layout is %s, shape is %s, expected shape is %s.", name.c_str(),
                    SFALayoutToSerialString(layout).c_str(), GetShapeStr(shape).c_str(),
...
template <typename T>
void SFATilingCheck::LogErrorNumberSupport(const std::vector<T> &expectNumberList, const T &actualValue,
...
template <typename T>
void SFATilingCheck::LogErrorDimNumSupport(const std::vector<T> &expectNumberList, const T &actualValue,
                                           const std::string &name) const
{
    LogErrorNumberSupport(expectNumberList, actualValue, name, "dimension");
}

ge::graphStatus SFATilingCheck::CheckDimNumInLayoutSupport(const SFALayout &layout, const gert::StorageShape *shape,
                                                           const std::string &name) const
{
                                           ...
                                           const SFALayout &actualLayout, const std::string &name) const
{
    ...
    OP_LOGE(opName_, "Tensor %s only supports layout %s, but got %s", name.c_str(), oss.str().c_str(),
            SFALayoutToSerialString(actualLayout).c_str());
}
...
ge::graphStatus SFATilingCheck::CheckSinglePara() const
{
    if (ge::GRAPH_SUCCESS != CheckSingleParaQuery() || ge::GRAPH_SUCCESS != CheckSingleParaKey() ||
        ...
        ge::GRAPH_SUCCESS != CheckSingleParaSparseBlockSize() || ge::GRAPH_SUCCESS != CheckSingleParaPreTokens() ||
...
ge::graphStatus SFATilingCheck::CheckRopeExistence()
{
    OP_CHECK_IF((opParamInfo_.queryRope.tensor != nullptr || opParamInfo_.keyRope.tensor != nullptr) &&
                ...
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(*opParamInfo_.attentionMode != 2, OP_LOGE(opName_, "attentionMode only support 2."),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF((opParamInfo_.queryRope.tensor != nullptr && opParamInfo_.keyRope.tensor == nullptr),
...
ge::graphStatus SFATilingCheck::CheckAttrValueByMap(std::map<std::string, std::pair<const T *, T>> &attrMap) const
{
    for (auto const &kv : attrMap) {
        ...
        if (*(pointerValuePair.first) != pointerValuePair.second) {
            ...
            OP_LOGE(opName_, "%s value should be %s, but got %s", name.c_str(), ossExpect.str().c_str(),
                    ossActual.str().c_str());
            return ge::GRAPH_FAILED;
        }
    }
...
ge::graphStatus SFATilingCheck::GetActualSeqLenSize(uint32_t &size, const gert::Tensor *tensor, const SFALayout &layout,
                                                    const std::string &name) const
{
    if (tensor == nullptr) {
        OP_LOGE(opName_, "when layout of query is %s, %s must be provided.", SFALayoutToSerialString(layout).c_str(),
                name.c_str());
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SFATilingCheck::CheckBlockTable() const
{
    ...
    OP_CHECK_IF(blockTableBatch != bSize_,
                OP_LOGE(opName_, "%s's first dimension(%u) should be equal to batch size(%u)", BLOCK_TABLE_NAME.c_str(),
...
ge::graphStatus SFATilingCheck::CheckDTypeConsistency(const ge::DataType &actualDtype, const ge::DataType &expectDtype,
                                                      const std::string &name) const
{
    if (actualDtype != expectDtype) {
        OP_LOGE(opName_, "%s dtype should be %s, but it's %s.", name.c_str(),
                SFADataTypeToSerialString(expectDtype).c_str(), SFADataTypeToSerialString(actualDtype).c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFATilingCheck::CheckAttenOut()
{
    if (ge::GRAPH_SUCCESS !=
        ...
        ge::GRAPH_SUCCESS != CheckAttenOutShape() || ge::GRAPH_SUCCESS != CheckSoftmaxMaxShape() ||
...
ge::graphStatus SFATilingCheck::CheckSoftmaxMax()
{
    if (*opParamInfo_.returnSoftmaxLse) {
        ...
        OP_CHECK_IF(opParamInfo_.softmaxMax.desc->GetDataType() != ge::DT_FLOAT,
                    OP_LOGE(opName_, "SoftmaxMax's dtype must be FLOAT."), return ge::GRAPH_FAILED);
        // shape和维度校验
...
ge::graphStatus SFATilingCheck::CheckSoftmaxSum()
{
    if (*opParamInfo_.returnSoftmaxLse) {
        ...
        OP_CHECK_IF(opParamInfo_.softmaxSum.desc->GetDataType() != ge::DT_FLOAT,
                    OP_LOGE(opName_, "softmaxSum's dtype must be FLOAT."), return ge::GRAPH_FAILED);
    } else {
...
ge::graphStatus SFATilingCheck::CheckVAndKRopeShapeForPageAttention()
{
    ...
    OP_CHECK_IF(blockNum <= 0, OP_LOGE(opName_, "The first dim(%ld) of key should be greater than 0", blockNum),
                return ge::GRAPH_FAILED);
    SFATilingShapeCompareParam shapeParams;
...
ge::graphStatus SFATilingCheck::CheckActualSeqLensQDType()
{
    ...
    if (opParamInfo_.actualSeqLengthsQ.desc == nullptr) {
        OP_LOGE(opName_,
                "actualSeqLengthsQ is not empty,"
...
ge::graphStatus SFATilingCheck::CheckActualSeqLensQShape()
{
    ...
    if (shapeSize != bSize_) {
        OP_LOGE(opName_, "actualSeqLengthsQ shape size is %u, it should be equal to batch size[%u]", shapeSize, bSize_);
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFATilingCheck::CheckActualSeqLensShape()
{
    ...
    if (shapeSize != bSize_) {
        OP_LOGE(opName_, "actualSeqLengths shape size is %u, it should be equal to batch size[%u].", shapeSize, bSize_);
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
{
    SetSFAShapeCompare();
    if (ge::GRAPH_SUCCESS != CheckVAndKRope() || ge::GRAPH_SUCCESS != CheckQRope() ||
        ge::GRAPH_SUCCESS != CheckTopK() || ge::GRAPH_SUCCESS != CheckAttenOut() ||
        ge::GRAPH_SUCCESS != CheckSoftmaxMax() || ge::GRAPH_SUCCESS != CheckSoftmaxSum() ||
...
ge::graphStatus SFATilingCheck::CheckFeatureMlaNoQuantShape() const
{
    ...
    OP_CHECK_IF(n1Size_ <= 0, OP_LOGE(opName_, "q_head_num should be greater than 0, but got %u", n1Size_),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(gSize_ < 1 || (gSize_ > 64 && gSize_ != 128),
                OP_LOGE(opName_, "group num should be in 1 ~ 64, 128, but got %u", gSize_), return ge::GRAPH_FAILED);

    OP_CHECK_IF(qkHeadDim_ != 512, OP_LOGE(opName_, "qk_head_dim only support 512, but got %u", qkHeadDim_),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(ropeHeadDim_ != 64, OP_LOGE(opName_, "rope_head_dim should be 64, but got %u", ropeHeadDim_),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFATilingCheck::CheckFeatureMlaNoquantPa() const
{
    ...
    OP_CHECK_IF(blockSize_ % 16 > 0,
                OP_LOGE(opName_, "when page attention is enabled, block_size(%d) should be 16-aligned.", blockSize_),
                return ge::GRAPH_FAILED);

    OP_CHECK_IF(blockSize_ % sparseBlockSize_ > 0,
                OP_LOGE(opName_,
                        "when page attention is enabled, block_size(%d) must be divided by sparse_block_size(%d), but "
                        "now the remainder is %d.",
                        blockSize_, sparseBlockSize_, blockSize_ % sparseBlockSize_),
                return ge::GRAPH_FAILED);
...
ge::graphStatus SFAInfoParser::CheckRequiredInOutExistence() const
{
    ...
    OP_CHECK_IF(opParamInfo_.sparseIndices.shape == nullptr,
                OP_LOGE(opName_, "Shape of tensor sparseIndices is nullptr"), return ge::GRAPH_FAILED);
...
ge::graphStatus SFAInfoParser::GetActualSeqLenSize(uint32_t &size, const gert::Tensor *tensor, SFALayout &layout,
                                                   const std::string &name) const
{
    if ((tensor == nullptr)) {
        OP_LOGE(opName_, "when layout of query is %s, %s must be provided.", SFALayoutToSerialString(layout).c_str(),
...
{
    const map<string, SFALayout> layoutKVMap = {{"BSND", SFALayout::BSND},
...
ge::graphStatus SFAInfoParser::GetMaxBlockNumPerBatch()
{
    if (opParamInfo_.blockTable.tensor == nullptr) {
        OP_LOGE(opName_, "the layout_kv is %s, blockTable must be provided.",
                SFALayoutToSerialString(kvLayout_).c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    if (opParamInfo_.blockTable.tensor->GetStorageShape().GetDim(1) <= 0) {
        OP_LOGE(opName_, "%s's second dimension(%ld) should be greater than 0", BLOCK_TABLE_NAME.c_str(),
                opParamInfo_.blockTable.tensor->GetStorageShape().GetDim(1));
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SFAInfoParser::GetRopeHeadDim()
{
    if (queryShape_.GetDimNum() != queryRopeShape_.GetDimNum()) {
        OP_LOGE(opName_,
                "The dimensions of query and query_rope should be equal, but query has dimension %zu while query_rope "
                "has dimension %zu.",
...
ge::graphStatus SFAInfoParser::Parse(SFATilingInfo &sfaInfo)
{
    ...
    if (ge::GRAPH_SUCCESS != GetInOutDataType() || ge::GRAPH_SUCCESS != GetQueryAndOutLayout() ||
        ge::GRAPH_SUCCESS != GetTopkLayout() || ge::GRAPH_SUCCESS != GetSoftmaxMaxAndSumLayout() ||
        ge::GRAPH_SUCCESS != GetKvLayout() || ge::GRAPH_SUCCESS != GetKvStorageMode()) {
        return ge::GRAPH_FAILED;
    }
    ...
    if (ge::GRAPH_SUCCESS != GetN1Size() || ge::GRAPH_SUCCESS != GetN2Size() || ge::GRAPH_SUCCESS != GetGSize() ||
        ...
        ge::GRAPH_SUCCESS != GetS1Size() || ge::GRAPH_SUCCESS != GetQkHeadDim() || ge::GRAPH_SUCCESS != GetS2Size() ||
        ge::GRAPH_SUCCESS != GetValueHeadDim() || ge::GRAPH_SUCCESS != GetRopeHeadDim() ||
